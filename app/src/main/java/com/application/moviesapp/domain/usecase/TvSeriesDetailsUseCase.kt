...
class GetTvSeriesDetailsInteractor @Inject constructor(private val repository: MoviesRepository) :
            ...
            return@withContext try {
                ...
                if (tvSeriesDetails.isSuccessful && tvSeriesCast.isSuccessful && tvSeriesGenre.isSuccessful) {
                    ...
                    val tvSeriesDetail = TvSeriesDetail(
                        ...
                        createdBy = tvSeriesDetails.body()?.createdBy?.map {
                            TvSeriesDetail.CreatedBy(
                                id = it?.id,
                        ...
                        lastEpisodeToAir = tvSeriesDetails.body()?.lastEpisodeToAir?.run {
                            TvSeriesDetail.LastEpisodeToAir(
                                airDate,
                                episodeNumber,
                                episodeType,
                                id,
                        ...
                        networks = tvSeriesDetails.body()?.networks?.map {
                            TvSeriesDetail.Network(
                                id = it?.id,
                        ...
                        nextEpisodeToAir = tvSeriesDetails.body()?.nextEpisodeToAir?.run {
                            TvSeriesDetail.NextEpisodeToAir(
                                airDate,
                                episodeNumber,
                                episodeType,
                                id,
                        ...
                        productionCompanies = tvSeriesDetails.body()?.productionCompanies?.map {
                            TvSeriesDetail.ProductionCompany(
                                id = it?.id,
                                logoPath = it?.logoPath,
                                name = it?.name,
                                originCountry = it?.originCountry
                        ...
                        productionCountries = tvSeriesDetails.body()?.productionCountries?.map {
                            TvSeriesDetail.ProductionCountry(
                                iso = it?.iso,
                                name = it?.name
                            )
                        },
                        seasons = tvSeriesDetails.body()?.seasons?.map {
                            TvSeriesDetail.Season(
                                airdate = it?.airdate,
                        ...
                        spokenLanguages = tvSeriesDetails.body()?.spokenLanguages?.map {
                            TvSeriesDetail.SpokenLanguage(
                                iso = it?.iso,
                                name = it?.name,
                        ...
                        cast = tvSeriesCast.body()?.cast?.map {
                            TvSeriesDetail.Cast(
                                adult = it?.adult,
                                gender = it?.gender,
                                id = it?.id,
                                ...
                                profilePath = it?.profilePath,
                                roles = it?.roles?.map { roles ->
                                    TvSeriesDetail.Cast.Role(
                                        character = roles?.character,
                                        creditId = roles?.creditId,
                                        episodeCount = roles?.episodeCount
                                    )
                                },
                                totalEpisodeCount = it?.totalEpisodeCount
                            )
                        },
                        crew = tvSeriesCast.body()?.crew?.map {
                            TvSeriesDetail.Crew(
                                adult = it?.adult,
                                gender = it?.gender,
                                id = it?.id,
                                ...
                                jobs = it?.jobs?.map { jobs ->
                                    TvSeriesDetail.Crew.Job(
                                        job = jobs?.job,
                                        creditId = jobs?.job,
                                        episodeCount = jobs?.episodeCount
                                    )
                                },
                                totalEpisodeCount = it?.totalEpisodeCount
                            )
