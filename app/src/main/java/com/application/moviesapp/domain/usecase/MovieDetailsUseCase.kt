...
class GetMovieDetailInteractor @Inject constructor(private val moviesRepository: MoviesRepository) :
    MovieDetailsUseCase {
            ...
            return@withContext try {
                ...
                if (movieDetailsOutput.isSuccessful && movieCastOutput.isSuccessful) {
                    ...
                    val result = MoviesDetail(
                        ...
                        belongsToCollection = MoviesDetail.Collection(
                            id = movieDetailsOutput.body()?.belongsToCollection?.id,
                            name = movieDetailsOutput.body()?.belongsToCollection?.name,
                            posterPath = movieDetailsOutput.body()?.belongsToCollection?.posterPath,
                            backdropPath = movieDetailsOutput.body()?.belongsToCollection?.backdropPath
                        ),
                        ...
                        productionCompanies = movieDetailsOutput.body()?.productionCompanies?.map {
                            MoviesDetail.ProductionCompany(
                                id = it?.id,
                        ...
                        productionCountries = movieDetailsOutput.body()?.productionCountries?.map {
                            MoviesDetail.ProductionCountry(
                                iso = it?.iso,
                                name = it?.name
                            )
                        },
                        ...
                        spokenLanguages = movieDetailsOutput.body()?.spokenLanguages?.map {
                            MoviesDetail.SpokenLanguage(
                        ...
                        cast = movieCastOutput.body()?.cast?.map {
                            MoviesDetail.Cast(
                                adult = it?.adult,
                                castId = it?.castId,
                        ...
                        crew = movieCastOutput.body()?.crew?.map {
                            MoviesDetail.Crew(
                                adult = it?.adult,
                                creditId = it?.creditId,
                                gender = it?.gender,
